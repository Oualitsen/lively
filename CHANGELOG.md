...
- **`AsyncValue<T>` sealed type** — new `AsyncLoading<T>`, `AsyncData<T>`, and `AsyncError<T>` classes shipped in the runtime and exported from `lively.dart`. Used as the state type for `Future<T>` and `Stream<T>` fields generated by the companion generator.
- **`@computed` annotation** — annotate a getter to opt into lazy cached computation. The generator wires dirty tracking so the value is recomputed at most once per reactive mutation cycle.
